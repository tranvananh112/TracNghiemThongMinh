// ============================================================================
// ROOM MANAGER - FIXED VERSION - Khắc phục lỗi tải bài từ thiết bị khác
// ============================================================================
    ...
    async startRoomQuiz(roomId) {
        try {
            ...
            if (!room.quiz || !room.quiz.questions || room.quiz.questions.length === 0) {
                ...
                console.error('Invalid quiz data:', room.quiz);
                return;
            }
            ...
            this.showToast('❌ Lỗi khi bắt đầu: ' + error.message, 'error');
        }
    }
...
// ⭐ THÊM FUNCTION KHÔI PHỤC QUIZ TỪ LOCALSTORAGE
function restoreQuizFromBackup() {
    try {
        const backupQuiz = localStorage.getItem('currentRoomQuiz');
        const backupRoom = localStorage.getItem('currentRoomData');
        
        if (backupQuiz && window.quizManager) {
            const quizData = JSON.parse(backupQuiz);
            window.quizManager.currentQuiz = quizData;
            window.quizManager._quizBackup = JSON.parse(JSON.stringify(quizData));
            console.log('✅ Quiz restored from localStorage backup');
            return true;
        }
        
        if (backupRoom && window.roomManager) {
            const roomData = JSON.parse(backupRoom);
            window.roomManager.currentRoom = roomData;
            console.log('✅ Room restored from localStorage backup');
        }
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ⭐ KHÔI PHỤC QUIZ KHI RELOAD TRANG
...
window.restoreQuizFromBackup = restoreQuizFromBackup;
